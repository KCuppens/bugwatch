...
export class BugwatchClient {
    ...
    if (!apiKey) {
      throw new Error(
        "BUGWATCH_API_KEY environment variable is required. " +
          "Set it to your Bugwatch API key."
      );
    }
    this.apiKey = apiKey;
    this.baseUrl = (
      process.env.BUGWATCH_URL || "https://api.bugwatch.dev"
    ).replace(/\/+$/, "");
  }
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Bugwatch API error ${response.status} ${response.statusText}: ${errorBody}`
      );
    }
  ...
  async getIssue(
    ...
    return this.request<ApiResponse<IssueDetail>>(
  ...
  async updateIssue(
    ...
    return this.request<ApiResponse<Issue>>(
      `/api/v1/projects/${projectId}/issues/${issueId}`,
      {
        method: "PATCH",
  ...
  async getIssueFrequency(
    projectId: string,
    issueId: string
  ): Promise<ApiResponse<IssueFrequency[]>> {
    return this.request<ApiResponse<IssueFrequency[]>>(
      `/api/v1/projects/${projectId}/issues/${issueId}/frequency`
    );
  }
  ...
  async getIssueEvent(
    ...
    return this.request<ApiResponse<EventDetail>>(
  ...
  async listMonitors(
    ...
    return this.request<PaginatedResponse<Monitor>>(
  ...
  async listServers(
    ...
    return this.request<ApiResponse<Server[]>>(
  ...
  async listAlertLogs(
    ...
    return this.request<AlertLog[]>(
      `/api/v1/projects/${projectId}/alerts/logs`
    );
  }
